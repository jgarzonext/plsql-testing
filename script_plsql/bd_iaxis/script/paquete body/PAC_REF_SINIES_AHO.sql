--------------------------------------------------------
--  DDL for Package Body PAC_REF_SINIES_AHO
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "AXIS"."PAC_REF_SINIES_AHO" AS
/******************************************************************************
   NOMBRE:     Pac_Ref_Sinies_Aho
   PROPÓSITO:     Funciones para siniestros de ahorro

   REVISIONES:
   Ver        Fecha        Autor             Descripción
   ---------  ----------  ---------------  ------------------------------------
   1.0        XX/XX/XXXX   XXX                1. Creación del package.
   2.0        27/04/2009   APD                3. Bug 9685 - en lugar de coger la actividad de la tabla seguros, llamar a la
...
******************************************************************************/
   ...
   BEGIN
      ...
      IF pcmotsin = 0 THEN
         -- Se busca si el asegurado fallecido tiene contratada la garantía adicional de fallecimiento
         BEGIN
            SELECT cgarant
              INTO vcgarant
              FROM garanseg
             WHERE sseguro = psseguro
               AND finiefe <= pfsinies
               AND(ffinefe > pfsinies
                   OR ffinefe IS NULL)
               AND nriesgo = 1
               ...
               AND NVL(f_pargaranpro_v(vcramo, vcmodali, vctipseg, vccolect, vcactivi, cgarant,
                                       'PROPIETARIO'),
                       0) = pnriesgo
               AND pac_ctaseguro.f_garan_suspendida(sseguro, cgarant) = 0;
         EXCEPTION
            WHEN OTHERS THEN
               vcgarant := NULL;   -- el asegurado no tiene contratada la garantia de cobertura adicional, por lo que no se debe crear  el sinietro
         END;
         ...
         num_err := pk_cal_sini.valo_pagos_sini(pfsinies, psseguro, vnsinies, vsproduc,
                                                vcactivi, 1, 1, pcmotsin, pfnotifi, vivalora,
                                                vipenali, vicapris);
         ...
         -- Si el asegurado tiene contratada alguna garantía de cobertura adicional de fallecimiento hay que dar de alta las valoraciones
         -- contra esta garantía para el siniestro dado de alta
         IF vcgarant IS NOT NULL THEN
            -- Se ejecutan las valoraciones contra la garantía vcgarant
            num_err := pk_cal_sini.valo_pagos_sini(pfsinies, psseguro, vnsinies, vsproduc,
                                                   vcactivi, vcgarant, 1, pcmotsin, pfnotifi,
            ...
            num_err := pac_sin_insert.f_insert_valoraciones(vnsinies, vcgarant, pfnotifi,
                                                            vivalora, vipenali, vicapris);
      ...
      ELSIF pcmotsin = 4 THEN
         -- Si el motivo de siniestro es 4.-Fallecimiento del 1er Titular, hay que dar de alta el siniestro por fallecimiento y otro
         -- en caso de que tenga la garantía de cobertura adicional contratada
         ...
         -- Si el asegurado tiene contratada alguna garantía de cobertura adicional de fallecimiento hay que dar de alta otro siniestro
         -- contra esta garantía
         IF vcgarant IS NOT NULL THEN
            -- Se Inicializa el siniestro
            num_err := pac_sin.f_inicializar_siniestro(psseguro, pnriesgo, pfsinies, pfnotifi,
                                                       ptsinies, 1, pcmotsin, 20, vnsinies,
                                                       vcgarant);
            ...
            -- Se inserta como destinatario el otro asegurado
            num_err := pac_sin_insert.f_insert_destinatarios(vnsinies, psseguro, pnriesgo, 1,
                                                             1, 1, NULL, NULL, vcbancar, NULL);
   ...
   EXCEPTION
      ...
      WHEN OTHERS THEN
         ocoderror := -999;
         omsgerror := 'Pac_Ref_Sinies_Aho.f_aperturar_siniestro: Error general en la función';
         p_tab_error(f_sysdate, f_user, 'Pac_Ref_Sinies_Aho.f_aperturar_siniestro', NULL,
   ...
   EXCEPTION
      WHEN OTHERS THEN
         ...
         omsgerror := f_literal(ocoderror, pcidioma_user);
         p_tab_error(f_sysdate, f_user, 'Pac_Ref_Sinies_Aho.f_valida_permite_rescate_total',
                     NULL,
...
******************************************************************************************/
      ...
      OPEN v_cursor FOR
         SELECT s.sproduc, s.cramo, s.cmodali, s.ctipseg, s.ccolect, s.npoliza, s.ncertif,
                ...
                DECODE(mostrar_datos,
                       ...
                       NULL) rend_bruto,
                DECODE(mostrar_datos, 1, 0, NULL) reduccion,
                DECODE(mostrar_datos, 1, res(1).iresred, NULL) reg_trans,
                DECODE(mostrar_datos,
                       1, GREATEST(res(1).iresrcm - res(1).iresred, 0),
                       NULL) rcm,
   ...
   EXCEPTION
      WHEN OTHERS THEN
         ...
         omsgerror := f_literal(ocoderror, pcidioma_user);
         p_tab_error(f_sysdate, f_user, 'Pac_Ref_Sinies_Aho.f_sim_rescate_total', NULL,
                     'parametros: psseguro=' || psseguro || ' pcagente=' || pcagente
                     || ' pfrescate =' || pfrescate,
                     SQLERRM);
...
********************************************************************************/
   ...
   EXCEPTION
      WHEN OTHERS THEN
         ocoderror := -999;
         omsgerror := 'Pac_Ref_Sinies_Aho.f_sol_rescate_total: Error general en la función';
         p_tab_error(f_sysdate, f_user, 'Pac_Ref_Sinies_Aho.f_sim_rescate_total', NULL,
                     'parametros: psseguro = ' || psseguro || '  pcagente = ' || pcagente
                     || 'pfrescate = ' || pfrescate,
                     SQLERRM);
...
END pac_ref_sinies_aho;

/

  GRANT EXECUTE ON "AXIS"."PAC_REF_SINIES_AHO" TO "CONF_DWH";
  GRANT EXECUTE ON "AXIS"."PAC_REF_SINIES_AHO" TO "PROGRAMADORESCSI";
  GRANT EXECUTE ON "AXIS"."PAC_REF_SINIES_AHO" TO "R_AXIS";
