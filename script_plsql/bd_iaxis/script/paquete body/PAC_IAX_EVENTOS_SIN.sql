--------------------------------------------------------
--  DDL for Package Body PAC_IAX_EVENTOS_SIN
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "AXIS"."PAC_IAX_EVENTOS_SIN" AS
/******************************************************************************
...
******************************************************************************/
   ...
   FUNCTION f_get_eventos(
      ...
      vparam         VARCHAR2(500)
         := 'pcevento:' || pcevento || ' ptevento:' || ptevento || ' pfinicio:' || pfinicio
            || ' pffinal:' || pffinal;
      vobject        VARCHAR2(50) := 'PAC_IAX_EVENTOS_SIN.f_get_eventos';
      vcursor        sys_refcursor;
      num_err        NUMBER;
   BEGIN
      num_err := pac_md_eventos_sin.f_get_eventos(UPPER(pcevento), ptevento, pfinicio,
                                                  pffinal, peventos, mensajes);
   ...
   *************************************************************************/
   FUNCTION f_get_evento(
      ...
      num_err        NUMBER;
   BEGIN
      num_err := pac_md_eventos_sin.f_get_evento(UPPER(pcevento), pcevento_out, pfinieve,
                                                 pffineve, pdeseventos, mensajes);
   ...
   FUNCTION f_set_codevento(
      ...
      vobject        VARCHAR2(50) := 'PAC_IAX_EVENTOS_SIN.f_set_codevento';
      num_err        NUMBER;
   BEGIN
      ...
      num_err := pac_md_eventos_sin.f_set_codevento(UPPER(pcevento), NVL(pfinieve, f_sysdate),
                                                    pffineve, mensajes);
   ...
   *************************************************************************/
   FUNCTION f_del_evento(pcevento IN VARCHAR2, mensajes OUT t_iax_mensajes)
      ...
      vpasexec       NUMBER(8) := 1;
      vparam         VARCHAR2(500) := 'pcevento:' || pcevento;
      vobject        VARCHAR2(50) := 'PAC_IAX_EVENTOS_SIN.f_del_evento';
      num_err        NUMBER;
   BEGIN
      IF pcevento IS NULL THEN
         RAISE e_param_error;
      ...
      num_err := pac_md_eventos_sin.f_del_evento(pcevento, mensajes);
   ...
   FUNCTION f_set_desevento(
      ...
      vparam         VARCHAR2(500)
         := 'pcevento:' || pcevento || ' pttiteve:' || pttiteve || ' ptevento:' || ptevento
            || ' pcidioma:' || pcidioma;
      vobject        VARCHAR2(50) := 'PAC_IAX_EVENTOS_SIN.f_set_desevento';
      num_err        NUMBER;
   BEGIN
      IF pcevento IS NULL
      ...
      num_err := pac_md_eventos_sin.f_set_desevento(UPPER(pcevento), pttiteve, ptevento,
                                                    pcidioma, mensajes);
