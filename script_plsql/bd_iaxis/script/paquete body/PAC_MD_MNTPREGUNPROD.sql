--------------------------------------------------------
--  DDL for Package Body PAC_MD_MNTPREGUNPROD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "AXIS"."PAC_MD_MNTPREGUNPROD" AS
/******************************************************************************
...
******************************************************************************/
   ...
   *************************************************************************/
   FUNCTION f_get_preguntas(
      ...
      vobject        VARCHAR2(200) := 'PAC_MD_MNTPREGUNPROD.f_get_preguntas';
   BEGIN
      texto := pac_mntpregunprod.f_get_preguntas(ptpregun,
                                                 NVL(pcidioma,
                                                     pac_md_common.f_get_cxtidioma()));
      vpasexec := 2;
      cur := pac_md_listvalores.f_opencursor(texto, mensajes);
      RETURN cur;
   EXCEPTION
   ...
   *************************************************************************/
   ...
   BEGIN
      ...
      -- Comprobamos si se puede asignar la pregunta
      v_error := pac_mntpregunprod.f_get_asignar(pcpregun, ptabla, psproduc, pcactivi,
                                                 pcgarant);
      ...
      v_error := pac_mntpregunprod.f_get_ctippre(pcpregun, pctippre);
      ...
      v_error := f_despregunta(pcpregun, NVL(pcidioma, pac_md_common.f_get_cxtidioma()),
                               pdescpreg);
   ...
   FUNCTION f_get_listrespue(
      ...
      vparam         VARCHAR2(500)
         := 'paràmetres - pcpregun: ' || pcpregun || ' - PCIDIOMA:' || pcidioma
            || ' - PCTIPPRE:' || pctippre;
      vobject        VARCHAR2(200) := 'PAC_MD_MNTPREGUNPROD.f_get_listrespue';
   BEGIN
      IF pcpregun IS NULL
      ...
      IF pctippre = 2 THEN   --Llista valors
         texto := pac_mntpregunprod.f_get_listrespue(pcpregun,
                                                     NVL(pcidioma,
                                                         pac_md_common.f_get_cxtidioma()));
         ...
         cur := pac_md_listvalores.f_opencursor(texto, mensajes);
      ELSIF pctippre = 1 THEN   --Si/No
         cur := pac_md_listvalores.f_detvalores(52, mensajes);
      END IF;
   ...
   FUNCTION f_set_pregunta(
      ...
      vparam         VARCHAR2(1000)
         := 'paràmetres - pcpregun:' || pcpregun || ' ptabla:' || ptabla || ' psproduc:'
            ...
            || ' pcrecarg:' || pcrecarg;
      vobject        VARCHAR2(200) := 'PAC_MD_MNTPREGUNPROD.f_set_pregunta';
   BEGIN
      IF pcpregun IS NULL
         OR psproduc IS NULL
         OR pcofersn IS NULL
      ...
      verror := pac_mntpregunprod.f_set_pregunta(pcpregun, ptabla, psproduc, pcactivi,
   ...
   *************************************************************************/
   FUNCTION f_get_pregunta(
      ...
      vparam         VARCHAR2(1000)
         := 'paràmetres - pcpregun:' || pcpregun || ' ptabla:' || ptabla || ' psproduc:'
            || psproduc || ' pcactivi:' || pcactivi || ' pcgarant:' || pcgarant;
      vobject        VARCHAR2(200) := 'PAC_MD_MNTPREGUNPROD.f_get_pregunta';
      pcpretip       NUMBER;
      pnpreord       NUMBER;
   ...
   BEGIN
      ...
      verror := pac_mntpregunprod.f_get_pregunta(pcpregun, ptabla, psproduc, pcactivi,
                                                 pcgarant, pcpretip, pnpreord, ptprefor,
                                                 pcpreobl, pnpreimp, pcresdef, pcofersn,
                                                 ptvalfor, pcmodo, pcnivel, pctarpol,
                                                 pcvisible, pcesccero, pcvisiblecol,
      ...
      IF ptabla = 'PROD' THEN
         propreguntas := ob_iax_prodpreguntas();
         propreguntas.cpregun := pcpregun;
         verror := f_despregunta(pcpregun, pac_md_common.f_get_cxtidioma(),
                                 propreguntas.tpregun);
         propreguntas.cpretip := pcpretip;
         propreguntas.npreord := pnpreord;
         propreguntas.tprefor := ptprefor;
         propreguntas.cpreobl := pcpreobl;
         propreguntas.npreimp := pnpreimp;
         propreguntas.cresdef := pcresdef;
         propreguntas.cofersn := pcofersn;
         propreguntas.tvalfor := ptvalfor;
         propreguntas.cmodo := pcmodo;
         propreguntas.cnivel := pcnivel;
         propreguntas.ctarpol := pctarpol;
         propreguntas.cvisible := pcvisible;
         propreguntas.cesccero := pcesccero;
         propreguntas.cvisiblecol := pcvisiblecol;
         propreguntas.cvisiblecert := pcvisiblecert;
         propreguntas.crecarg := pcrecarg;
      ELSIF ptabla = 'ACT' THEN
         pactpreguntas := ob_iax_prodpregunacti();
         pactpreguntas.cpregun := pcpregun;
         verror := f_despregunta(pcpregun, pac_md_common.f_get_cxtidioma(),
                                 pactpreguntas.tpregun);
         pactpreguntas.cpretip := pcpretip;
         pactpreguntas.npreord := pnpreord;
         pactpreguntas.tprefor := ptprefor;
         pactpreguntas.cpreobl := pcpreobl;
         pactpreguntas.npreimp := pnpreimp;
         pactpreguntas.cresdef := pcresdef;
         pactpreguntas.cofersn := pcofersn;
         pactpreguntas.tvalfor := ptvalfor;
      ELSIF ptabla = 'GAR' THEN
         pgarpreguntas := ob_iax_prodpregunprogaran();
         pgarpreguntas.cpregun := pcpregun;
         verror := f_despregunta(pcpregun, pac_md_common.f_get_cxtidioma(),
                                 pgarpreguntas.tpregun);
         pgarpreguntas.cpretip := pcpretip;
         pgarpreguntas.npreord := pnpreord;
         pgarpreguntas.tprefor := ptprefor;
         pgarpreguntas.cpreobl := pcpreobl;
         pgarpreguntas.npreimp := pnpreimp;
         pgarpreguntas.cresdef := pcresdef;
         pgarpreguntas.cofersn := pcofersn;
         pgarpreguntas.tvalfor := ptvalfor;
         pgarpreguntas.cesccero := pcesccero;
         pgarpreguntas.cvisiblecol := pcvisiblecol;
         pgarpreguntas.cvisiblecert := pcvisiblecert;
         pgarpreguntas.cvisible := pcvisible;
         pgarpreguntas.cmodo := pcmodo;
      END IF;
   ...
   *************************************************************************/
   FUNCTION f_del_pregunta(
      ...
      vparam         VARCHAR2(1000)
         := 'paràmetres - pcpregun:' || pcpregun || ' ptabla:' || ptabla || ' psproduc:'
            || psproduc || ' pcactivi:' || pcactivi || ' pcgarant:' || pcgarant;
      vobject        VARCHAR2(200) := 'PAC_MD_MNTPREGUNPROD.f_del_pregunta';
   BEGIN
      IF pcpregun IS NULL
         OR ptabla IS NULL
         OR psproduc IS NULL THEN
      ...
      verror := pac_mntpregunprod.f_del_pregunta(pcpregun, ptabla, psproduc, pcactivi,
                                                 pcgarant);
