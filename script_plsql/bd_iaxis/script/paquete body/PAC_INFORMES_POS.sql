--------------------------------------------------------
--  DDL for Package Body PAC_INFORMES_POS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "AXIS"."PAC_INFORMES_POS" IS
   v_txt          VARCHAR2(32000);

/******************************************************************************
 NOMBRE: AXIS_D72.PAC_INFORMES_POS


   REVISIONES:
   Ver        Fecha        Autor             Descripción
   ---------  ----------  ---------------  ------------------------------------
   1.0        04/02/2013  ETM              1.0024776: (POSDE500)-Desarrollo-GAPS - Siniestros - Id 173 - Criterios de filtro listados
   2.0        04/03/2013  JGR              2.0025615: POSPG100-(POSPG100)-Parametrizacion- Administracion y Finanzas- Parametrizacion Cierres
   3.0        18/04/2013  ETM              3.0024933: POSRE100-(POSR100)-Informes y reportes-Administracion y finanzas
   4.0        02/05/2013  JMF              4.0025623 (POSDE200)-Desarrollo-GAPS - Comercial - Id 56
   5.0        18/09/2012  ETM              5.0025615: POSPG100-(POSPG100)-Parametrizacion- Administracion y Finanzas- Parametrizacion Cierres
   6.0        04/11/2013  JGR              6.0028785: Error en los cálculos de Días Amortizados y Días por Amortizar
   ...
   9.0        16/04/2015  JDS              9.0035599: POSES400-FUNCIONALIDAD LIBRO RADICADOR Y LIBROS AUXILIARES
  ...
  12.0        03/06/2015  IPH             12.0035149  0035149: POSADM Reporte de Provisión - modificacióm maps 662 y 664
******************************************************************************/

   /******************************************************************************************
     Descripció: Funció que genera texte capçelera per llistat dinamic siniestros abiertos en un periodo
...
--BUG 24776 -- 04/02/2013--ETM --INI
   FUNCTION f_list655_cab(
      ...
      RETURN VARCHAR2 IS
      v_tobjeto      VARCHAR2(100) := 'PAC_INFORMES_POS.f_list655_cab';
   ...
   FUNCTION f_list655_det(
      ...
      p_sperson IN NUMBER DEFAULT NULL)
      RETURN VARCHAR2 IS
      v_tobjeto      VARCHAR2(100) := 'PAC_INFORMES_POS.f_list655_det';
      v_tparam       VARCHAR2(1000)
         := ' idioma=' || p_cidioma || ' e=' || p_cempres || ' cagrpro=' || p_cagrpro
            || ' cramo=' || p_cramo || ' p_sproduc=' || p_sproduc || ' p_fdesde=' || p_fdesde
            || ' p_fhasta=' || p_fhasta || ' sucursal=' || p_sucursal || ' p_ccausin='
   ...
   END f_list655_det;
    ...
    /******************************************************************************************
        Descripció: Funció que genera texte capçelera per llistat dinamic SINIESTROS SEGUN SITUACION
        Paràmetres entrada:    - p_cidioma     -> codigo idioma
                               - p_cempres     -> codigo empresa
                               - p_cagrpro     ->
                               - p_cramo       -> ramo
   ...
   FUNCTION f_list656_cab(
      ...
      p_sperson IN NUMBER DEFAULT NULL)
      RETURN VARCHAR2 IS
      v_tobjeto      VARCHAR2(100) := 'PAC_INFORMES_POS.f_list656_cab';
      v_tparam       VARCHAR2(1000)
         := ' idioma=' || p_cidioma || ' e=' || p_cempres || ' cagrpro=' || p_cagrpro
   ...
   /******************************************************************************************
       Descripció: Funció que genera texte capçelera per llistat de pagos de sinestros por estado
       Paràmetres entrada:    - p_cidioma     -> codigo idioma
                              - p_cempres     -> codigo empresa
                              - p_cagrpro     ->
                              - p_cramo       -> ramo
   ...
   FUNCTION f_list657_cab(
      ...
      p_sperson IN NUMBER DEFAULT NULL)
      RETURN VARCHAR2 IS
      v_tobjeto      VARCHAR2(100) := 'PAC_INFORMES_POS.f_list657_cab';
      v_tparam       VARCHAR2(1000)
         := ' idioma=' || p_cidioma || ' e=' || p_cempres || ' cagrpro=' || p_cagrpro
   ...
   /******************************************************************************************
        Descripció: Funció que genera el contigut per  llistat de pagos liquidados en un periodo (map 657 dinamic)
        Paràmetres entrada:  - p_cidioma     -> codigo idioma
                            - p_cempres     -> codigo empresa
                            - p_cagrpro     ->
                            - p_cramo       -> ramo
   ...
   FUNCTION f_list657_det(
      ...
      RETURN VARCHAR2 IS
      -- ':CIDIOMA|:CEMPRES|:CAGRPRO|:CRAMO|:SPRODUC|:CAGENTE|:FDESDE|:FHASTA|:SUCURSAL|:CCAUSIN|:CMOTSIN|:SPROFES|:TIPO_PERS|:SPERSON',
      v_tobjeto      VARCHAR2(100) := 'PAC_INFORMES_POS.f_list657_det';
      v_tparam       VARCHAR2(1000)
         := ' idioma=' || p_cidioma || ' e=' || p_cempres || ' cagrpro=' || p_cagrpro
            || ' cramo=' || p_cramo || ' p_sproduc=' || p_sproduc || ' p_cagente='
            || p_cagente || ' p_fdesde=' || p_fdesde || ' p_fhasta=' || p_fhasta
...
--' || v_idioma || '
   ...
   /******************************************************************************************
       Descripció: Funció que genera la cebeceta per llistat pagos de siniestros por estado(map 658 dinamic)
       Paràmetres entrada:  - p_cidioma     -> codigo idioma
                           - p_cempres     -> codigo empresa
   ...
   FUNCTION f_list658_cab(
      ...
      p_sperson IN NUMBER DEFAULT NULL)
      RETURN VARCHAR2 IS
      v_tobjeto      VARCHAR2(100) := 'PAC_INFORMES_POS.f_list658_cab';
      v_tparam       VARCHAR2(1000)
         := ' idioma=' || p_cidioma || ' e=' || p_cempres || ' cagrpro=' || p_cagrpro
   ...
   /******************************************************************************************
         Descripció: Funció que genera el contigut per llistat pagos de siniestros por estado(map 658 dinamic)(map 658 dinamic)
         Paràmetres entrada:  - p_cidioma     -> codigo idioma
                             - p_cempres     -> codigo empresa
                             - p_cagrpro     -> codigo agrup produ
                             - p_cramo       -> codigo ramo
   ...
   FUNCTION f_list658_det(
      ...
      p_ctipdes IN NUMBER DEFAULT NULL)
      RETURN VARCHAR2 IS
      v_tobjeto      VARCHAR2(100) := 'PAC_INFORMES_POS.f_list658_det';
      v_tparam       VARCHAR2(1000)
         := ' idioma=' || p_cidioma || ' e=' || p_cempres || ' cagrpro=' || p_cagrpro
            || ' cramo=' || p_cramo || ' p_sproduc=' || p_sproduc || ' p_cestpag='
            || p_cestpag || ' p_fdesde=' || p_fdesde || ' p_fhasta=' || p_fhasta
...
--FIN BUG 24776 -- 04/02/2013--ETM --
   ...
   FUNCTION f_661_cab(pcempres IN VARCHAR2)
      RETURN VARCHAR2 IS
      v_tobjeto      VARCHAR2(100) := 'PAC_INFORMES_POS.f_661_cab';
      v_tparam       VARCHAR2(1000);
      v_ntraza       NUMBER := 0;
      v_sep          VARCHAR2(1) := ';';
...
select
       ...
       ||f_axis_literales(9001875,x.idi)||'';'' -- Póliza
       ||f_axis_literales(101168,x.idi)||'';'' -- Certificado
       ||f_axis_literales(111274,x.idi)||'';'' -- Duración:
       ||f_axis_literales(100582,x.idi)||'';'' -- Efecte
       ||f_axis_literales(100829,x.idi)||'';'' -- Producto
       ||f_axis_literales(101672,x.idi)||'';'' -- Prima anual
       ||f_axis_literales(180063,x.idi)||'';'' -- Cap. Defunció
       ||f_axis_literales(107118,x.idi)||'';'' -- Capital garantit
       ||f_axis_literales(107120,x.idi)||'';'' -- Provisió matemàtica
       ||f_axis_literales(107049,x.idi)||'';'' -- Interés Técnico
...
';
   ...
   FUNCTION f_661_det(
      ...
      RETURN VARCHAR2 IS
      v_tobjeto      VARCHAR2(100) := 'PAC_INFORMES_POS.f_661_det';
      v_tparam       VARCHAR2(1000);
      v_ntraza       NUMBER := 0;
      v_sep          VARCHAR2(1) := ';';
      v_ret          VARCHAR2(1) := CHR(10);
...
pac_corretaje.f_impcor_agente(vdr.iprinet + vdr.irecfra,agco.cagente,z.sseguro,z.nmovimi)|| '';'' || -- pneta,
...
pac_isqlfor.f_telefono(z.sperson)|| '';'' ||pac_isqlfor_pos.f_direccion(z.sperson,1)|| '';'' || pac_isqlfor.f_dirpais(z.sperson,1,x.v_idioma) || '';'' ||
...
nvl(agco.ppartici,100)||'';''||pac_propio_pos.ff_pcomisi_pcomisi(z.sseguro, z.nrecibo, NULL)|| '';'' || ff_desvalorfijo(1053,x.v_idioma,b.ctipben)|| '';'' ||
...
SELECT s.cempres,s.cagente,u.nmovimi,r.sseguro,r.ctipcoa,ff_descompania(pac_cuadre_adm.f_es_vida(s.sseguro)) tempres,  ff_desramo(u.cramo,8) cramo, u.cmodali,
       ...
       r.femisio femisio, r.fefecto fefecto, r.fvencim fvencim, u.fcalcul fcalcul, u.finiefe finiefe, sum(u.iprovmora) iprovmora,nvl(r.sperson,t.sperson) sperson
        FROM pppc_pos u, seguros s, tomadores t, asegurados a, recibos r, empresas e, per_personas p, per_personas p2
        WHERE s.sseguro = u.sseguro
         ...
         || ' AND u.cerror = 0
           ...
           AND r.ctiprec <> 9
           --IPH 35149 -- outer join en la tabla menos informada
           --and t.sseguro = s.sseguro
           and t.sseguro = s.sseguro
           and a.sseguro(+) = s.sseguro
           and a.norden (+) = 1
        ...
        GROUP BY s.cempres,s.cagente,u.nmovimi,r.sseguro,s.npoliza, u.nrecibo,
           ...
           --SUBSTR(pac_redcomercial.f_busca_padre(s.cempres,s.cagente, NULL, NULL),-3),
       ...
       UNION ALL
        ...
        FROM pppc_pos_previ u, seguros s, recibos r, tomadores t,asegurados a, empresas e, per_personas p, per_personas p2
        WHERE s.sseguro = u.sseguro
           AND u.nrecibo = r.nrecibo
                      AND u.fcalcul = TO_DATE('''
         || fecha_calcul || ''', ''ddmmyyyy'')
...
and nvl(agco.nmovimi,-1) =  (select nvl(max(x.nmovimi),-1) from age_corretaje x where x.sseguro = z.sseguro and x.nmovimi <= z.nmovimi)';
   ...
   /******************************************************************************************
     Descripció: f_664_det Detalle del map 664
     return:     texto cabecera
   ******************************************************************************************/
   FUNCTION f_664_det(fecha_calcul IN VARCHAR2, cproces IN VARCHAR2, cempres IN VARCHAR2)
      RETURN VARCHAR2 IS
      v_tobjeto      VARCHAR2(100) := 'PAC_INFORMES_POS.f_664_det';
      v_tparam       VARCHAR2(1000);
      v_ntraza       NUMBER := 0;
      v_sep          VARCHAR2(1) := ';';
      v_ret          VARCHAR2(1) := CHR(10);
...
SELECT s.cempres,s.cagente,u.nmovimi,r.sseguro,r.ctipcoa,ff_descompania(pac_cuadre_adm.f_es_vida(s.sseguro)) tempres, ff_desramo(u.cramo,8) cramo, u.cmodali,
       ...
       sum(u.iprovmora)-(select nvl(sum(iprovmora),0) from pppc_pos where nrecibo = u.nrecibo and fcalcul = (select max (fcalcul) from pppc_pos where fcalcul < u.fcalcul)) imorac, nvl(r.sperson,t.sperson) sperson
        FROM pppc_pos u, seguros s, tomadores t, asegurados a, recibos r, empresas e, per_personas p, per_personas p2
        WHERE s.sseguro = u.sseguro
           AND u.nrecibo = r.nrecibo
                      AND u.fcalcul = TO_DATE('''
       ...
       0-sum(u.iprovmora) imorac, nvl(r.sperson,t.sperson) sperson
        ...
        WHERE s.sseguro = u.sseguro
         ...
         || fecha_calcul
         || ''',''ddmmyyyy'')
             AND u.sproces = (select max(sproces) from pppc_pos where sproces < '
         || cproces
         || ')
...
-- BUG 0025623 - 02/05/2013 - JMF: afegir pperage
 /******************************************************************************************
       Descripció: Función que genera el texto cabecera para el Listado de Recibos segun situacion
       Paràmetres entrada:    - p_cidioma     -> codigo idioma
                              - p_cempres     -> codigo empresa
   ...
   FUNCTION f_list733_cab(
      ...
      RETURN VARCHAR2 IS
      v_tobjeto      VARCHAR2(100) := 'PAC_INFORMES_POS.f_list733_cab';
      v_tparam       VARCHAR2(1000)
   ...
   BEGIN
      ...
      v_txt :=
         ...
         || '|| f_axis_literales(100784, x.v_idioma) || '';''   --ramo
               ...
               || f_axis_literales(9905357, x.v_idioma) || '';''   --antigüedad de cartera
   ...
   FUNCTION f_list733_det(
      ...
      p_perage IN NUMBER DEFAULT NULL)
      RETURN VARCHAR2 IS
      v_tobjeto      VARCHAR2(100) := 'PAC_INFORMES_POS.f_list733_det';
      v_tparam       VARCHAR2(1000)
         := ' idioma=' || p_cidioma || ' empresa=' || p_cempres || ' cestrec=' || p_cestrec
            || ' fdesde=' || p_fdesde || ' p_fhasta=' || p_fhasta || ' p_ctiprec= '
            || p_ctiprec || ' p_cagente=' || p_cagente || ' p_cestado=' || p_cestado
...
--   ' || v_idioma || '
      v_txt := v_txt || 'SELECT se.npoliza ' || v_sep2 || ' se.ncertif ' || v_sep2
               ...
               || ' ff_desvalorfijo(1026, ' || v_idioma || ', NVL(re.ctipcob, 2)) ' || v_sep2
               || ' e.tempres ' || v_sep2 || ' re.nrecibo ' || v_sep2
               || ' ff_desagente(re.cagente) ' || v_sep2 || ' re.fefecto ' || v_sep2
            --INI RAL BUG 37852: POS ADM Listado de Administración - Recibos por Estado Actual
               --|| ' re.femisio ' || v_sep2
               || '(SELECT FMOVDIA FROM MOVRECIBO WHERE NRECIBO = re.nrecibo AND cestrec=0 AND cestant=0)' || v_sep2
            --FIN RAL BUG 37852: POS ADM Listado de Administración - Recibos por Estado Actual
               || ' re.fvencim ' || v_sep2
               || ' ff_desvalorfijo(8, ' || v_idioma || ', re.ctiprec) ' || v_sep2
               || ' ff_desvalorfijo(61, ' || v_idioma || ', se.csituac) ' || v_sep2
...
-- BUG 0025623 - 02/05/2013 - JMF: afegir pperage
    /******************************************************************************************
       Descripció: Función que genera el texto cabecera para el Listado de Recibos pendientes por poliza
       Paràmetres entrada:    - p_cidioma     -> codigo idioma
                              - p_cempres     -> codigo empresa
                              - p_cestrec     -> esstado de recibo
                              - p_fdesde       ->  fecha ini
   ...
   FUNCTION f_list734_cab(
      ...
      p_perage IN NUMBER DEFAULT NULL)
      RETURN VARCHAR2 IS
      v_tobjeto      VARCHAR2(100) := 'PAC_INFORMES_POS.f_list734_cab';
      v_tparam       VARCHAR2(1000)
         := ' idioma=' || p_cidioma || ' empresa=' || p_cempres || ' cestrec=' || p_cestrec
            || ' fdesde=' || p_fdesde || ' p_fhasta=' || p_fhasta || ' p_ctiprec= '
            || p_ctiprec || ' p_cagente=' || p_cagente || ' p_cestado=' || p_cestado
   ...
   FUNCTION f_list734_det(
      ...
      p_perage IN NUMBER DEFAULT NULL)
      RETURN VARCHAR2 IS
      v_tobjeto      VARCHAR2(100) := 'PAC_INFORMES_POS.f_list734_det';
      v_tparam       VARCHAR2(1000)
         := ' idioma=' || p_cidioma || ' empresa=' || p_cempres || ' cestrec=' || p_cestrec
            || ' fdesde=' || p_fdesde || ' p_fhasta=' || p_fhasta || ' p_ctiprec= '
            || p_ctiprec || ' p_cagente=' || p_cagente || ' p_cestado=' || p_cestado
   ...
   BEGIN
      ...
      v_txt :=
         ...
         || ' AND m.fmovfin IS NULL '   -- el último movimiento
         || ' AND re.cagente = a.cagente '   -- filtramos que solo pueda ver por su nivel de visión
         || ' AND re.cempres = a.cempres '   -- filtramos que solo pueda ver por su nivel de visión
         || ' AND(re.cagente, re.cempres) IN( SELECT rc.cagente, rc.cempres '
         || ' FROM (SELECT rc.cagente, rc.cempres ' || ' FROM redcomercial rc '
         || ' WHERE rc.fmovfin IS NULL ' || ' START WITH ' || v_cagente2
         || ' CONNECT BY PRIOR rc.cagente = rc.cpadre '
...
--FIN--BUG 24933 -- 18/04/2013--ETM --
   /******************************************************************************************
       Descripció: Funció que genera la capçalera per llistat sobrecomisions
       Paràmetres entrada:  - p_cidioma     -> codigo idioma
                           - p_cempres     -> codigo empresa
                           - p_mes         -> mes
                           - p_anyo         -> año
   ...
   FUNCTION f_list752_cab(
      ...
      p_cagente IN NUMBER DEFAULT NULL)
      RETURN VARCHAR2 IS
      v_tobjeto      VARCHAR2(100) := 'PAC_INFORMES_POS.f_list752_cab';
      v_tparam       VARCHAR2(1000)
   ...
   /******************************************************************************************
         Descripció: Funció que genera el contigut per llistat sobrecomisions
          Paràmetres entrada:  - p_cidioma     -> codigo idioma
                           - p_cempres     -> codigo empresa
                           - p_mes         -> mes
                           - p_anyo         -> año
   ...
   FUNCTION f_list752_det(
      ...
      p_cagente IN NUMBER DEFAULT NULL)
      RETURN VARCHAR2 IS
      v_tobjeto      VARCHAR2(100) := 'PAC_INFORMES_POS.f_list752_det';
      v_tparam       VARCHAR2(1000)
         := ' idioma=' || p_cidioma || ' e=' || p_cempres || ' mes=' || p_mes || ' any='
            || p_anyo || ' ramo=' || p_cramo || ' agente=' || p_cagente;
      v_ntraza       NUMBER := 0;
...
--BUG 0025615 -- 18/09/2012--ETM --INI
   FUNCTION f_663_cab
      RETURN VARCHAR2 IS
      v_tobjeto      VARCHAR2(100) := 'PAC_INFORMES_POS.f_663_cab';
      v_tparam       VARCHAR2(1000);
      v_ntraza       NUMBER := 0;
      v_sep          VARCHAR2(1) := ';';
      v_ret          VARCHAR2(1) := CHR(10);
   ...
   FUNCTION f_663_det(pfcalcul VARCHAR2, psproces NUMBER, pcempres NUMBER)
      RETURN VARCHAR2 IS
      v_tobjeto      VARCHAR2(100) := 'PAC_INFORMES_POS.f_663_det';
      v_tparam       VARCHAR2(1000);
      v_ntraza       NUMBER := 0;
      v_sep          VARCHAR2(1) := ';';
      v_ret          VARCHAR2(1) := CHR(10);
...
--        pac_corretaje.f_impcor_agente(nvl(u.inupr_moncon, u.inupr),agco.cagente,s.sseguro,u.nmovimi) inupr,
        ...
        s.cactivi, s.ncertif, 0 prim_no_con, pac_corretaje.f_impcor_agente(nvl(u.icwp_moncon, u.icwp),agco.cagente,s.sseguro,u.nmovimi) icwp, u.fini, u.ffin,'
         ||
            --6.0028785: Error en los cálculos de Días Amortizados y Días por Amortizar - Inicio
         ...
         ||
            --6.0028785: Error en los cálculos de Días Amortizados y Días por Amortizar - Final
         '0 prim_dev_coa,
        0 prim_no_dev_coa
...
--FIN - BUG 0025615 -- 18/09/2012--ETM
   ...
   /******************************************************************************************
       Descripció: Funció que genera la capçalera pel llistat de salaris
       Paràmetres entrada:  - p_cidioma     -> codigo idioma
                           - p_cempres     -> codigo empresa
                           - p_mes         -> mes
                           - p_anyo         -> año
   ...
   FUNCTION f_list756_cab1(
      ...
      p_cagente IN NUMBER DEFAULT NULL)
      RETURN VARCHAR2 IS
      v_tobjeto      VARCHAR2(100) := 'PAC_INFORMES_POS.f_list756_cab1';
      v_tparam       VARCHAR2(1000)
         := ' idioma=' || p_cidioma || ' e=' || p_cempres || ' mes=' || p_mes || ' any='
            || p_anyo || ' ramo=' || p_cramo || ' agente=' || p_cagente;
      v_ntraza       NUMBER := 0;
   ...
   /******************************************************************************************
         Descripció: Funció que genera el contigut pel llistat salaris
          Paràmetres entrada:  - p_cidioma     -> codigo idioma
                           - p_cempres     -> codigo empresa
                           - p_mes         -> mes
                           - p_anyo         -> año
   ...
   FUNCTION f_list756_det1(
      ...
      p_cagente IN NUMBER DEFAULT NULL)
      RETURN VARCHAR2 IS
      v_tobjeto      VARCHAR2(100) := 'PAC_INFORMES_POS.f_list756_det1';
      v_tparam       VARCHAR2(1000)
         := ' idioma=' || p_cidioma || ' e=' || p_cempres || ' mes=' || p_mes || ' any='
            || p_anyo || ' ramo=' || p_cramo || ' agente=' || p_cagente;
      v_ntraza       NUMBER := 0;
   ...
   BEGIN
      ...
      v_txt := ' select sp.CAGENTE' || v_sep2 || 'sp.ISALARIO' || v_sep2 || 'sp.PCUMPLI'
               ...
               || v_sep3 || ' linea ' || ' from pos_sal_result_previo sp '
      ...
      v_txt := v_txt || ' UNION  select sp.CAGENTE' || v_sep2 || 'sp.ISALARIO' || v_sep2
               ...
               || ' sp.PPORSAL' || v_sep3 || ' linea ' || ' from pos_sal_result sp  '
               || ' where  sp.fcierre between TO_DATE(' || CHR(39) || v_fini || CHR(39)
               || ', ''dd/mm/yyyy'')' || ' and TO_DATE(' || CHR(39) || v_ffin || CHR(39)
               || ', ''dd/mm/yyyy'') ';
   ...
   FUNCTION f_list756_det2(
      ...
      p_cagente IN NUMBER DEFAULT NULL)
      RETURN VARCHAR2 IS
      v_tobjeto      VARCHAR2(100) := 'PAC_INFORMES_POS.f_list756_det2';
      v_tparam       VARCHAR2(1000)
         := ' idioma=' || p_cidioma || ' e=' || p_cempres || ' mes=' || p_mes || ' any='
            || p_anyo || ' ramo=' || p_cramo || ' agente=' || p_cagente;
      v_ntraza       NUMBER := 0;
   ...
   BEGIN
      ...
      v_txt := v_txt || ' union all select sp.CAGENTE' || v_sep2 || ' sp.CAGEORI ' || v_sep2
               ...
               || ' linea '
               || ' from pos_sal_detalle_previo sp, productos p, titulopro t, ramos ra '
               || ' where  sp.fcierre between TO_DATE(' || CHR(39) || v_fini || CHR(39)
               || ', ''dd/mm/yyyy'')' || ' and TO_DATE(' || CHR(39) || v_ffin || CHR(39)
      ...
      v_txt := v_txt || ' UNION all select sp.CAGENTE' || v_sep2 || ' sp.CAGEORI ' || v_sep2
               || ' ra.TRAMO' || v_sep2 || 't.TTITULO' || v_sep2 || ' sp.IIMPORTE ' || v_sep3
               || ' linea ' || ' from pos_sal_detalle sp, productos p, titulopro t, ramos ra '
               || ' where  sp.fcierre between TO_DATE(' || CHR(39) || v_fini || CHR(39)
               || ', ''dd/mm/yyyy'')' || ' and TO_DATE(' || CHR(39) || v_ffin || CHR(39)
               || ', ''dd/mm/yyyy'')' || ' and sp.cramo = t.cramo and  t.cramo = p.cramo '
               || ' and t.cmodali = p.cmodali ' || ' and t.ctipseg = p.ctipseg '
   ...
   FUNCTION f_list777_cabecera1a(
      ...
      p_sproduc IN NUMBER DEFAULT NULL)
      RETURN VARCHAR2 IS
      v_tobjeto      VARCHAR2(100) := 'PAC_INFORMES_POS.f_list777_cabecera1a';
      v_tparam       VARCHAR2(1000)
   ...
   FUNCTION f_list777_linea1a(
      ...
      p_sproduc IN NUMBER DEFAULT NULL)
      RETURN VARCHAR2 IS
      v_tobjeto      VARCHAR2(100) := 'PAC_INFORMES_POS.f_list777_linea1a';
      v_tparam       VARCHAR2(1000)
         := ' sproces=' || p_sproces || ' cgarant=' || p_cgarant || ' p_sproduc=' || p_sproduc;
      v_ntraza       NUMBER := 0;
      v_finiefe      VARCHAR2(100);
   ...
   *****************************************************************************************/
   FUNCTION f_518_cab
      RETURN VARCHAR2 IS
      v_tobjeto      VARCHAR2(100) := 'PAC_INFORMES_POS.f_518_CAB';
      v_tparam       VARCHAR2(1000);
      v_ntraza       NUMBER := 0;
      v_sep          VARCHAR2(1) := ';';
      v_ret          VARCHAR2(1) := CHR(10);
   ...
   BEGIN
      v_ttexto :=
         'SELECT
                f_axis_literales(800279 ,x.idi)  || '';'' || /* 1 Número de siniestro */
                f_axis_literales(9906505 ,x.idi) || '';'' || /* 2 Numero de siniestros */
                f_axis_literales(101606 ,x.idi)  || '';'' || /* 3 Año ejercicio */
                f_axis_literales(9905622 ,x.idi) || '';'' || /* 4 Número de Póliza */
                ...
                f_axis_literales(9907594, x.idi) || '';'' || /* 29b Nº Movimientos del siniestro */
   ...
   *****************************************************************************************/
   FUNCTION f_518_det(
      ...
      psproduc IN NUMBER)
      RETURN VARCHAR2 IS
      v_tobjeto      VARCHAR2(100) := 'PAC_INFORMES_POS.f_518_DET';
      v_tparam       VARCHAR2(1000)
   ...
   BEGIN
      ...
      v_ttexto :=
         ...
         || ', (pac_informes_pos.F_GET_ESTADOMOVRESERVA(str.nsinies, str.ntramit, str.ctipres, str.nmovres, str.cgarant, str.idres)))'
         || v_sep ||   /*30*/
                    'ff_desvalorfijo(1143, ' || pcidioma
         || ', (pac_informes_pos.F_GET_CAUSAMODRESERVA(str.nsinies, str.ntramit, str.ctipres, str.nmovres, str.cgarant, str.idres)))'
         || v_sep ||   /*31*/
                    'si.iperit' || v_sep ||   /*32*/
                                           'si.cagente' || v_sep
         ||   /*33*/
           'decode(s.ctiprea, 0, 0, f_round(str.ireserva_moncia * ((100 - pac_informes_pos.f_plocal_rea_coa(si.sseguro, si.fsinies, si.nriesgo, str.cgarant)) / 100)))'
         || v_sep
         ||   /*34*/
           'f_round(str.ireserva_moncia * (pac_informes_pos.f_plocal_rea_coa(si.sseguro, si.fsinies, si.nriesgo, str.cgarant) / 100))'
         || v_sep
         ||   /*35*/
                ...
                WHERE p2.sperson = stpr.sperson
                  ...
                  AND stpr.npersrel = (SELECT MIN(npersrel)
                                        ...
                                        WHERE stpr2.nsinies = stpr.nsinies
                                          ...
                                          AND stpr2.ctiprel = stpr.ctiprel)))'
         || v_sep ||   /*45*/
                    'PAC_INFORMES_POS.f_descausin(si.ccausin , ' || pcidioma || ')' || v_sep
         ||   /*46*/
           'PAC_INFORMES_POS.f_desmotsin(si.ccausin , si.cmotsin, ' || pcidioma || ')' || v_sep
         ||   /*46b*/
           'stp_pagos.nfacref' || v_sep
         ||   /*47*/
           'NVL2(str.sidepag, f_round(NVL(stp_pagos.isinretpag,0) * ((100 - pac_informes_pos.f_plocal_rea_coa(si.sseguro, si.fsinies, si.nriesgo, str.cgarant)) / 100)), NULL)'
         || v_sep ||   /*48*/
                    'str.cusualt '   /*49*/
   ...
   /*****************************************************************************************
      ...
      se determina que tipo de operación se ha realizado con la reserva.
   ...
   *****************************************************************************************/
   FUNCTION f_get_causamodreserva(
      ...
      pidres IN NUMBER)
      RETURN NUMBER IS
      v_tobjeto      VARCHAR2(500) := 'pac_informes_pos.F_get_causamodreserva';
   ...
   BEGIN
      ...
      */
      ...
      /*
         Liberacion de reserva, cmovres = 6, 7 ó 8
      */
      v_ntraza := 5;
      ...
      */
      ...
      IF v_movact.ireserva > v_movant.ireserva THEN
         v_codi := 02;   -- Aumento generioco

         IF v_movact.ctipres = 3 THEN
            v_codi := 3;   -- Aumento por gatos
   ...
   *****************************************************************************************/
   FUNCTION f_get_estadomovreserva(
      ...
      pidres IN NUMBER)
      RETURN NUMBER IS
      v_tobjeto      VARCHAR2(500) := 'pac_informes_pos.F_get_estadomovreserva';
      v_tparam       VARCHAR2(500)
         := 'pnsinies: ' || pnsinies || ' pntramit ' || pntramit || ' pctipres ' || pctipres
            || ' pnmovres ' || pnmovres || ' pcgarant ' || pcgarant || ' pidres ' || pidres;
      v_ntraza       NUMBER := 1;
      v_ccaumod      NUMBER;
      v_valor        NUMBER := 12;   -- Per defecte 'Modificación reserva'
   BEGIN
      v_ntraza := 1;
   ...
   *****************************************************************************************/
   FUNCTION f_esamparodemuerte(psproduc IN NUMBER, pcgarant IN NUMBER)
      RETURN NUMBER IS
      v_tobjeto      VARCHAR2(500) := 'pac_informes_pos.F_esamparodemuerte';
   ...
   *****************************************************************************************/
   FUNCTION f_descausin(pccausin IN NUMBER, pcidioma IN NUMBER)
      RETURN VARCHAR2 IS
      v_tobjeto      VARCHAR2(500) := 'pac_informes_pos.F_descausin';
   ...
   *****************************************************************************************/
   FUNCTION f_desmotsin(pccausin IN NUMBER, pcmotsin IN NUMBER, pcidioma IN NUMBER)
      RETURN VARCHAR2 IS
      v_tobjeto      VARCHAR2(500) := 'pac_informes_pos.F_desmotsin';
      v_tparam       VARCHAR2(500)
   ...
   *****************************************************************************************/
   FUNCTION f_get_reserva_rea_moncia(
      ...
      v_tobjeto      VARCHAR2(100) := 'PAC_INFORMES_POS.f_get_reserva_rea_moncia';
      v_tparam       VARCHAR2(1000);
      v_ntraza       NUMBER := 0;
      vsum           NUMBER := 0;
      vtotal         NUMBER := 0;
   ...
   *****************************************************************************************/
   FUNCTION f_get_cesiones_moncia(pnsinies IN VARCHAR2, pfdesde IN DATE, pfhasta IN DATE)
      RETURN NUMBER IS
      v_tobjeto      VARCHAR2(100) := 'PAC_INFORMES_POS.f_get_cesiones_moncia';
      v_tparam       VARCHAR2(1000);
      v_ntraza       NUMBER := 0;
      vsum           NUMBER := 0;
      vtotal         NUMBER := 0;
   ...
   *****************************************************************************************/
   FUNCTION f_plocal_coa(
      ...
      RETURN NUMBER IS
      v_tobjeto      VARCHAR2(100) := 'PAC_INFORMES_POS.f_plocal_coa';
      v_ntraza       NUMBER;
      v_tparam       VARCHAR2(1000)
   ...
   *****************************************************************************************/
   FUNCTION f_plocal_rea(
      ...
      pcgarant IN sin_tramita_reserva.cgarant%TYPE)
      RETURN NUMBER IS
      v_tobjeto      VARCHAR2(100) := 'PAC_INFORMES_POS.f_plocal_rea';
      v_ntraza       NUMBER;
      v_tparam       VARCHAR2(1000)
         := 'psseguro: ' || psseguro || ' pfsinies: ' || pfsinies || ' pnriesgo: ' || pnriesgo
            || ' pcgarant: ' || pcgarant;
      v_plocal_total NUMBER;   -- Total a retornar
      v_plocal_ctr_reten NUMBER;   -- Plen net de retenció
      v_plocal_facult NUMBER;   -- Facultatiu
      v_plocal_ctr_qp NUMBER;   -- contracte QT i d'altres amb retenció.
      v_plocal_coaseguro NUMBER;   -- Coaseuro
   ...
   *****************************************************************************************/
   FUNCTION f_plocal_rea_coa(
      ...
      pcgarant IN sin_tramita_reserva.cgarant%TYPE)
      RETURN NUMBER IS
      v_tobjeto      VARCHAR2(100) := 'PAC_INFORMES_POS.f_plocal_rea_coa';
      v_ntraza       NUMBER;
      v_tparam       VARCHAR2(1000)
         := 'psseguro: ' || psseguro || ' pfsinies: ' || pfsinies || ' pnriesgo: ' || pnriesgo
            || ' pcgarant: ' || pcgarant;
...
END pac_informes_pos;

/

  GRANT EXECUTE ON "AXIS"."PAC_INFORMES_POS" TO "R_AXIS";
  GRANT EXECUTE ON "AXIS"."PAC_INFORMES_POS" TO "CONF_DWH";
  GRANT EXECUTE ON "AXIS"."PAC_INFORMES_POS" TO "PROGRAMADORESCSI";
