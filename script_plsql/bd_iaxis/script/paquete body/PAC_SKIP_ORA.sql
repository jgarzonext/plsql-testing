--------------------------------------------------------
--  DDL for Package Body PAC_SKIP_ORA
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "AXIS"."PAC_SKIP_ORA" IS
   /******************************************************************************
      NOMBRE:       PAC_SKIP_ORA
   ...
   *************************************************************************/
   ...
   EXCEPTION
      WHEN OTHERS THEN
         p_tab_error(f_sysdate(), NVL(f_user(), 'AXIS'), 'pac_skip_ora.f_indexAssociat', NULL
                   , 'OTHERS', SQLERRM);
         RETURN(NULL);
   END f_indexassociat;
   ...
   *************************************************************************/
   ...
   EXCEPTION
      WHEN OTHERS THEN
         p_tab_error(f_sysdate, NVL(f_user(), 'AXIS'), 'pac_skip_ora.f_comprovadrop', NULL
                   , 'OTHERS', SQLERRM);
   ...
   *************************************************************************/
   ...
   EXCEPTION
      WHEN OTHERS THEN
         p_tab_error(f_sysdate, NVL(f_user(), 'AXIS'), 'pac_skip_ora.f_ComprovaConstraint'
                   , NULL, 'OTHERS', SQLERRM);
         DBMS_OUTPUT.put_line('ORA-11217 ' || SQLERRM);
         RETURN -1;
   ...
   *************************************************************************/
   PROCEDURE p_insertliteral(
    ...
    , pi_texte IN VARCHAR
    , pi_slitera IN NUMBER
    ,   -- codi literal
      pi_clitera IN NUMBER DEFAULT 1) IS   -- tipus de literal ( només per forms)
      --
      w_user         VARCHAR2(100);
      w_resultat     NUMBER := 0;
      w_tidioma      idiomas.tidioma%TYPE;
   --
   BEGIN
      --
      BEGIN
         ...
         SELECT tidioma
           INTO w_tidioma
           FROM idiomas
          WHERE cidioma = pi_cidioma;

         --
         -- Esborrem el literal per evitar que existeixi.
         DELETE FROM axis_literales
               WHERE slitera = pi_slitera;
         ...
         INSERT INTO axis_codliterales
                     (slitera, clitera)
              VALUES (pi_slitera, pi_clitera);

         INSERT INTO axis_literales
                     (cidioma, slitera, tlitera)
              VALUES (pi_cidioma, pi_slitera, pi_texte);
      --
      EXCEPTION
         WHEN NO_DATA_FOUND THEN
            ...
            -- 'BSBVAL', 'BSBPROD'
            IF w_user IN('APRA_V', 'AXIS')
               AND pi_cidioma = 1 THEN
               DBMS_OUTPUT.put_line('Idioma:' || pi_cidioma || ' propietari:' || w_user
                                    || ' => OK');
               w_resultat := 1;
            ELSE
               w_resultat := -1;
            ...
            --
            p_tab_error(f_sysdate, NVL(f_user(), 'AXIS'), 'pac_skip_ora.f_InsertLiteral', NULL
                      , 'Idioma:' || pi_cidioma || ' propietari:' || w_user || ' => OK'
                      , SQLERRM);
         WHEN OTHERS THEN
            p_tab_error(f_sysdate, NVL(f_user(), 'AXIS'), 'pac_skip_ora.f_InsertLiteral'
                      , NULL, 'OTHERS: Error Insert axis_codliterales o axis_literales.'
                      , SQLERRM);
            -- Si no existeix l'idioma sortim, no cal fer res.
            w_resultat := -1;
            RAISE;
      END;
   --
   EXCEPTION
      WHEN OTHERS THEN
         p_tab_error(f_sysdate, NVL(f_user(), 'AXIS'), 'pac_skip_ora.f_InsertLiteral', NULL
                   , 'OTHERS.Error no controlat fa raise.Idioma:' || pi_cidioma
                     || ', propietari:' || w_user
                   , SQLERRM);
         DBMS_OUTPUT.put_line('ORA-11217 ' || SQLERRM);
   --
   ...
   *************************************************************************/
   ...
   EXCEPTION
      WHEN OTHERS THEN
         p_tab_error(f_sysdate, NVL(f_user(), 'AXIS'), 'pac_skip_ora.f_ComprovaColumn', NULL
