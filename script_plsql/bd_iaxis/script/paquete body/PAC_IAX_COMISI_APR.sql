--------------------------------------------------------
--  DDL for Package Body PAC_IAX_COMISI_APR
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "AXIS"."PAC_IAX_COMISI_APR" AS
   ...
   FUNCTION f_obtener_polizas(
      ...
      v_param        VARCHAR2(200)
         := 'p_cagente=' || p_cagente || ' p_tagente=' || p_tagente || ' p_npoliza='
            ...
            || ' p_hasta=' || p_hasta;
      v_object       VARCHAR2(200) := 'PAC_IAX_COMISI_APR.F_OBTENER_POLIZAS';
      v_numerr       NUMBER(10);
   BEGIN
      v_numerr := pac_md_comisi_apr.f_obtener_polizas(p_cagente, p_tagente, p_npoliza,
   ...
   FUNCTION f_obtener_total_comis(
      ...
      v_object       VARCHAR2(200) := 'PAC_IAX_COMISI_APR.F_OBTENER_TOTAL_COMIS';
      v_numerr       NUMBER(10);
   BEGIN
      ...
      v_numerr := pac_md_comisi_apr.f_obtener_total_comis(p_sseguro, p_npoliza, p_fefecto,
   ...
   FUNCTION f_obtener_comisiones(
      ...
      v_pasexec      NUMBER(8) := 1;
      v_param        VARCHAR2(200) := 'p_sseguro=' || p_sseguro;
      v_object       VARCHAR2(200) := 'PAC_IAX_COMISI_APR.F_OBTENER_COMISIONES';
      v_numerr       NUMBER(10);
   BEGIN
      IF p_sseguro IS NULL THEN
         RAISE e_param_error;
      ...
      v_numerr := pac_md_comisi_apr.f_obtener_comisiones(p_sseguro, p_refcursor, mensajes);
      RETURN v_numerr;
   EXCEPTION
      WHEN e_param_error THEN
         pac_iobj_mensajes.p_tratarmensaje(mensajes, v_object, 1000006, v_pasexec, v_param);
