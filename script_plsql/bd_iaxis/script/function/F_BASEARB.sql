--------------------------------------------------------
--  DDL for Function F_BASEARB
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE FUNCTION "AXIS"."F_BASEARB" (
   psseguro IN NUMBER,
   pfdata IN DATE,
   pnriesgo IN NUMBER,
   pnerror OUT NUMBER)
...
--
-- ALLIBADM. OBTÉ LA BASE PEL CÀLCUL DELS ARBITRIS.
--
--
...
BEGIN
   IF psseguro IS NOT NULL
      ...
      WHILE cur_garant%FOUND LOOP
         ...
         IF xcimparb = 1 THEN   -- S' OBTÉ LA BASE PEL CÀLCUL DELS ARBITRIS
            BEGIN
               SELECT SUM(NVL(ipritot, 0))   --(YSR) COASEGURO CEDIDO RECIBO UNICO (IPRIANU)
                 INTO xiprima
                 FROM garanseg
                WHERE sseguro = psseguro
                  AND cgarant = xcgarant
...
/
  ...
  GRANT EXECUTE ON "AXIS"."F_BASEARB" TO "R_AXIS";
  GRANT EXECUTE ON "AXIS"."F_BASEARB" TO "CONF_DWH";
  GRANT EXECUTE ON "AXIS"."F_BASEARB" TO "PROGRAMADORESCSI";
