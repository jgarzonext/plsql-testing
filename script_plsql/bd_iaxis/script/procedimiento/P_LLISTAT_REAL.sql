--------------------------------------------------------
--  DDL for Procedure P_LLISTAT_REAL
--------------------------------------------------------
set define off;

  CREATE OR REPLACE EDITIONABLE PROCEDURE "AXIS"."P_LLISTAT_REAL" (
...
AUTHID CURRENT_USER IS
   ...
   CURSOR c_liquid(vagente IN NUMBER) IS
      SELECT   DECODE(cdebhab, 1, 'Debe', 2, 'Haber') cdebhab, d.cconcta, d.tcconcta, iimport,
          ...
          FROM ctactes, desctactes d
         WHERE sproces = p_sproliq
           AND cagente = vagente
           AND d.cconcta = ctactes.cconcta
...
/
  ...
  GRANT EXECUTE ON "AXIS"."P_LLISTAT_REAL" TO "R_AXIS";
  GRANT EXECUTE ON "AXIS"."P_LLISTAT_REAL" TO "CONF_DWH";
  GRANT EXECUTE ON "AXIS"."P_LLISTAT_REAL" TO "PROGRAMADORESCSI";
