...
BEGIN
  UPDATE map_tabla m
     SET m.tfrom =
      '(SELECT nvl(sperson_deud,sperson) linea FROM per_personas WHERE sperson IN (SELECT p.sperson_rel linea FROM per_pagador_alt p WHERE p.sperson = pac_map.f_valor_parametro(''|'',''#lineaini'',101,''#cmapead'') AND p.cestado = 1))'
