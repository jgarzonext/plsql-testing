--------------------------------------------------------
--  DDL for View ESTCONTACTOS
--------------------------------------------------------

  CREATE OR REPLACE FORCE EDITIONABLE VIEW "AXIS"."ESTCONTACTOS" ("SPERSON", "CAGENTE", "CAGENTEPROD", "CMODCON", "CTIPCON", "TCOMCON", "TVALCON", "SWPUBLI", "CPREFIX") AS 
  SELECT c.sperson, c.cagente, ff_agenteprod() cagenteprod, c.cmodcon, c.ctipcon, c.tcomcon,
          c.tvalcon, p.swpubli, c.cprefix
     FROM estper_contactos c, estper_personas p
   ...
   SELECT c.sperson, c.cagente, ff_agenteprod() cagenteprod, c.cmodcon, c.ctipcon, c.tcomcon,
          c.tvalcon, p.swpubli, c.cprefix
     FROM estper_personas p, estper_contactos c, estper_detper d
    ...
    WHERE p.sperson = c.sperson
      ...
      AND c.cagente IN(SELECT     r.cagente
                       ...
                       CONNECT BY PRIOR cagente = cpadre
                              AND PRIOR fmovfin IS NULL)
;
  GRANT UPDATE ON "AXIS"."ESTCONTACTOS" TO "R_AXIS";
  GRANT SELECT ON "AXIS"."ESTCONTACTOS" TO "R_AXIS";
  GRANT INSERT ON "AXIS"."ESTCONTACTOS" TO "R_AXIS";
  GRANT DELETE ON "AXIS"."ESTCONTACTOS" TO "R_AXIS";
  GRANT SELECT ON "AXIS"."ESTCONTACTOS" TO "CONF_DWH";
  GRANT SELECT ON "AXIS"."ESTCONTACTOS" TO "PROGRAMADORESCSI";
